...
func (w *Worker) worker(powDigest []byte, startNonce uint64, target uint, done *uint32, counter *uint64) (uint64, error) {
	if target > consts.HashTrinarySize {
...
func checkStateTrits(l, h *[consts.HashTrinarySize]uint, n uint) int {
	var v uint
	for i := consts.HashTrinarySize - n; i < consts.HashTrinarySize; i++ {
